...
- `url`: a URL of a wearable or an emote to load. If it is a wearable, it will override anything loaded from a profile. It can be used many times. The url will be fetched and must return a valid definition following the [`WearableDefinition`](https://github.com/memetaverseproject/common-schemas/blob/main/src/dapps/preview/wearable-definition.ts) or [`EmoteDefinition`](https://github.com/memetaverseproject/common-schemas/blob/main/src/dapps/preview/emote-definition.ts) types.
...
- `bodyShape`: which body shape to use, possible values are `urn:memetaverse:off-chain:base-avatars:BaseMale` or `urn:memetaverse:off-chain:base-avatars:BaseFemale`.
...
Example: https://wearable-preview.memetaverse.club?contract=0xce163f74accd4ec7912224216348d77fce06ae51&item=0
...
```ts
import { PreviewMessageType, sendMessage } from '@mtvproject/schemas'
...
```ts
import { PreviewMessageType, PreviewMessagePayload } from '@mtvproject/schemas'
...
import { PreviewMessageType, PreviewMessagePayload, sendMessage } from '@mtvproject/schemas'
