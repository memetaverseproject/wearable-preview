...
const genderPaths = {[BodyShape.MALE]: "male", [BodyShape.FEMALE]: "female"}
...
export function buildEmoteUrl(config: PreviewConfig) {
  let baseUrl = process.env.PUBLIC_URL || ''
  if (!baseUrl.endsWith('/')) {
    baseUrl += '/'
  }
  const path = `./emotes/${genderPaths[config.bodyShape]}/${config.emote}.glb`
...
export async function playEmote(scene: Scene, assets: Asset[], config: PreviewConfig) {
  ...
  if (!container && config.emote) {
    const emoteUrl = buildEmoteUrl(config)
