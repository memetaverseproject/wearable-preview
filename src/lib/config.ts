...
async function fetchURNs(urns: string[], peerUrl: string): Promise<[WearableDefinition[], EmoteDefinition[]]> {
  ...
  const sanitizedAssetUrns = urns
    .map((urn: string) => urn.replace(/^mtv:\/\/base-avatars\//, 'urn:memetaverse:off-chain:base-avatars:'))
...
async function fetchItemFromContract(options: {
  ...
  let urn = `urn:memetaverse:${network}:collections:${contractAddress}:${itemId}`
  if (!itemId && !tokenId) {
    ...
    urn =
      nft.network !== Network.ETHEREUM
        ? `urn:memetaverse:${network}:collections:${contractAddress}:${nft.itemId}`
